...
class KittiDataset(Det3DDataset):
    ...
    def __init__(self,
                 ...
                 pcd_limit_range: List[float] = [-1e10, -1e10, -1e10, 1e10, 1e10, 1e10],
                 **kwargs) -> None:
    ...
    def parse_ann_info(self, info: dict) -> dict:
        ...
        # proj_mat = np.array(info['images']['CAM2']['cam2img'])

        gt_bboxes_3d = CameraInstance3DBoxes(ann_info['gt_bboxes_3d'])
        
        # points_2d, bboxes_2d = CameraInstance3DBoxes(
        #     ann_info['gt_bboxes_3d']).bbox_2d(calib=proj_mat, 
        #                                       h=info['images']['CAM2']['height'], 
        #                                       w=info['images']['CAM2']['width'])

        # ann_info['gt_bboxes_2d'] = bboxes_2d


        ann_info['gt_bboxes_2d'] = ann_info['gt_bboxes_2d']
        ann_info['frame'] = ann_info['frame']
        ann_info['calib_matrices'] = ann_info['calib_matrices']
        ann_info['gt_bboxes_3d'] = gt_bboxes_3d

        # gt_centers_3d_expanded = np.zeros((ann_info['gt_bboxes_3d'].shape[0], 7))
        # gt_centers_3d_expanded[:, :3] = ann_info['gt_bboxes_3d']
        # gt_centers_3d_expanded_img = CameraInstance3DBoxes(
        #     gt_centers_3d_expanded).convert_to(self.box_mode_3d,
        #                                          np.linalg.inv(lidar2cam))
        # ann_info['gt_centers_3d'] = gt_centers_3d_expanded_img
